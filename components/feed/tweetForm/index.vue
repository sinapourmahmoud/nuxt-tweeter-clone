  ...
  <form @submit.prevent="addTweet">
    ...
    >
      <div class="flex gap-5">
        ...
        <textarea
          v-model="input"
          class="w-full outline-none border-none bg-transparent rounded-lg dark:text-white dark:placeholder:text-white"
          placeholder="Enter a tweet"
        ></textarea>
      </div>
      <p v-if="isError" class="text-red-500 text-sm font-medium">
        Please add a text
      </p>
      <img
        :src="dbLink"
        alt="poster"
        v-if="dbLink"
        class="w-full object-cover max-h-80 rounded-2xl"
      />
      ...
      <div class="ml-16 flex items-center justify-between">
        <div class="flex items-center w-full justify-between my-2">
          <div
            @click="clickEvent"
            class="text-blue-300 dark:text-blue-50 dark:hover:bg-dim-100 cursor-pointer p-2 rounded-full hover:bg-blue-100"
